...
class VkOauth:
    ...
    async def _get_vk_access_token(vk_access_data):
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://id.vk.com/oauth2/auth", data=vk_access_data
            ) as response:
    ...
    async def _get_vk_user_info(self, access_token):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                "https://id.vk.com/oauth2/user_info", data=data_for_user_info
            ) as response:
    ...
    async def get_vk_user(self, device_id, state, code):
        vk_access_data = self._get_vk_access_data(
            device_id=device_id, state=state, code=code
        )

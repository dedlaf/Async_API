...
from services.mongo_service import get_mongo_crud, MongoCRUD
...
async def create_user(user: User, mongo_crud: MongoCRUD = Depends(get_mongo_crud)):
    user_dict = user.dict()
    user_id = await mongo_crud.insert_document(collection_name='users', data=user_dict)
...
async def get_user(user_id: str, mongo_crud: MongoCRUD = Depends(get_mongo_crud)):
    ...
    found_user = await mongo_crud.find(collection_name='users', condition={"_id": obj_id})
    if not found_user:
...
async def update_user(
    ...
    obj_id = ObjectId(user_id)
    await mongo_crud.update_document(collection_name='users', condition={"_id": obj_id}, new_values=user.dict())
    updated_user = await mongo_crud.find(collection_name='users', condition={"_id": obj_id})
...
async def delete_user(user_id: str, mongo_crud: MongoCRUD = Depends(get_mongo_crud)):
    obj_id = ObjectId(user_id)
    result = await mongo_crud.delete_document(collection_name='users', condition={"_id": obj_id})
...
async def get_users(mongo_crud: MongoCRUD = Depends(get_mongo_crud)):
    found_users = await mongo_crud.find(collection_name='users', condition={}, multiple=True)

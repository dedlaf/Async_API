...
async def check_access_token(cookies: dict):
    async with aiohttp.ClientSession() as session:
        async with session.get("http://nginx:80/api/v1/auth/user", cookies=cookies) as response:
            return response.status


async def check_refresh_token(cookies: dict):
    async with aiohttp.ClientSession() as refresh_session:
        async with refresh_session.get("http://nginx:80/api/v1/auth/refresh", cookies=cookies) as refresh_response:
    ...
    async def wrapper(*args, **kwargs):
        ...
        if await check_access_token(cookies) is not HTTPStatus.OK.real:
            ...
            if refresh_status is HTTPStatus.OK.real:
                rs.set_cookie(key="access_token_cookie", value=refreshed_cookies.get("access_token_cookie").value)
                rs.set_cookie(key="refresh_token_cookie", value=refreshed_cookies.get("refresh_token_cookie").value)
